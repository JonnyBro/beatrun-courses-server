	...
	if (req.user) {
		...
		codes.forEach(code => {
			if (courses[code].uploader.userid === req.user.steamid) {
				...
				userCourses.push(
					`<a class="hover:text-red-700" href="/stats/${code}">${courses[code].name}</a>`,
				)
			}
		})
...
async function hasGame(locals, user) {
	const response = (
		await fetch(
			`https://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=${locals.config.steamKey}&steamid=${user.steamid}&format=json`,
		).then(r => r.json())
	).response
...
async function registerUser(locals, user) {
	if (
		Math.floor((new Date() - new Date(user.timecreated * 1000)) / (1000 * 60 * 60 * 24 * 30)) <
		3
	)
