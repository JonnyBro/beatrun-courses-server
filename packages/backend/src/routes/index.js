...
router.get('/', async (req, res) => {
	...
	codes.forEach(async code => {
		...
		if (!isCourseFileValid(parsedCodeFile))
			console.log(`[WARNING] Course is not valid: ${code}`)
		...
		const codeMapId =
			codeData.mapid !== '0' && codeData.mapid !== '' ? codeData.mapid : '2818480138' // le troll
	...
	const sortedCards = cards
		.slice(Math.max((page - 1) * 20 - 1, 0), Math.max((page - 1) * 20 - 1, 0) + 20)
		.join('\n')
...
function generateCourseCard(course) {
	const templates = {
		...
		"{uploadDate}": new Date(course.time).toLocaleString("ru-RU", {
			day: "2-digit",
			month: "2-digit",
			year: "2-digit",
		}),
