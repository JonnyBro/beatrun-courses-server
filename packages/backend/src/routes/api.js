...
router.get('/download', isUserGame, async (req, res) => {
	...
	if (!headers.code)
		return res
			...
			.json({
				res: res.statusCode,
				message: 'No code provided. Please provide a valid course code.',
			})
	if (!headers.map)
		return res
			.status(401)
			.json({ res: res.statusCode, message: 'No map provided. Please provide a valid map.' })
	...
	if (ip !== 'Unknown' && (await isRatelimited(db, ip)))
		return res
			.status(401)
			.json({ res: res.statusCode, message: 'Too many requests. Please try again later.' })
	if (ip !== 'Unknown' && (await isMultiAccount(db, ip, steamid)))
		return res
			.status(401)
			.json({
				res: res.statusCode,
				message:
					'Your account was detected as multiaccount. Please open a ticket on our Discord server.',
			})
	if (await isLocked(db, steamid))
		return res
			.status(401)
			.json({
				res: res.statusCode,
				message: 'Your account is locked. Please open a ticket on our Discord server.',
			})
	...
	try {
		...
		return res
			.status(401)
			.json({ res: res.statusCode, message: 'Invalid course code provided.' })
	}

	if (courseData.map !== headers.map)
		return res
			.status(401)
			.json({
				res: res.statusCode,
				message: 'Invalid map. Please start a map that require by course you provided.',
			})
...
router.post('/upload', isUserGame, async (req, res) => {
	...
	if (!body || !body.data)
		return res
			...
			.json({
				res: res.statusCode,
				message: 'No course provided. Please provide a valid course.',
			})
	if (!headers.map)
		return res
			.status(401)
			.json({ res: res.statusCode, message: 'No map provided. Please provide a valid map.' })
	if (headers.mapid === null || headers.mapid === undefined)
		return res
			.status(401)
			.json({
				res: res.statusCode,
				message: 'No map id provided. Please provide a valid map id.',
			})
	...
	if (!isCourseFileValid(JSON.parse(course)))
		return res
			.status(401)
			.json({
				res: res.statusCode,
				message: 'Invalid course file. Please provide a valid course.',
			})
	...
	const mapImage =
		headers.mapid === '0' || headers.mapid === 'no_map_id'
			? ''
			: await openGraphScraper({
					url: `https://steamcommunity.com/sharedfiles/filedetails/?id=${headers.mapid}`,
				}).then(data => data.result.ogImage[0].url)
...
router.post('/upload_site', isUser, async (req, res) => {
	...
	if (ip !== 'Unknown' && (await isRatelimited(db, ip)))
		return res
			.status(401)
			.json({ res: res.statusCode, message: 'Too many requests. Please try again later.' })
	if (ip !== 'Unknown' && (await isMultiAccount(db, ip, user.steamid)))
		return res
			.status(401)
			.json({
				res: res.statusCode,
	...
	if (await isLocked(db, user.steamid))
		return res
			.status(401)
			.json({
				res: res.statusCode,
	...
	form.parse(req, async (err, fields, files) => {
		if (err)
			return res
				.status(401)
				.send('Error while uploading file. Please contact the administrator.')
		...
		if (!isCourseFileValid(JSON.parse(course)))
			return res.status(401).send('Invalid course file. Please provide a valid course.')
...
router.post('/update', isUserGame, async (req, res) => {
	...
	if (ip !== 'Unknown' && (await isRatelimited(db, ip)))
		return res.status(401).json({ message: 'Too many requests. Please try again later.' })
	if (ip !== 'Unknown' && (await isMultiAccount(db, ip, steamid)))
		return res
			.status(401)
			.json({
	...
	if (courseData.map !== headers.map)
		return res
			...
			.json({
				res: res.statusCode,
				message: 'Invalid map. You should provide the same map as before.',
			})
	if (courseData.uploader.userid !== steamIds[key])
		return res
			.status(401)
			.json({
				res: res.statusCode,
				message:
					'Invalid key. You are not the uploader of this course. Only the uploader can update their course.',
			})
...
router.post('/rate', isUser, async (req, res) => {
	...
	if (action === 'like') {
		...
		res.send({
			success: true,
			code: code,
			likes: Object.values(ratings[code]).filter(x => x === true).length,
		})
		...
		res.send({
			success: true,
			code: code,
			dislikes: Object.values(ratings[code]).filter(x => x === false).length,
		})
...
router.post('/admin', isAdmin, async (req, res) => {
	...
	if (action === 'addKey') {
		if (!target)
			return res.send({
				success: false,
				message: 'Target not provided. Please provide a target.',
			})

		const key = await getKey(req.app.locals.db, target)

		if (!key)
			return res.send({ success: false, message: 'Internal error. Contact the developer.' })
		...
		if (!courses[target.toUpperCase()])
			return res.send({ success: false, message: 'Invalid course code provided.' })
