...
async function isAdmin(req, res, next) {
	...
	if (!admins[steamid])
		return res.status(401).json({ res: res.statusCode, message: 'Unauthorized.' })
...
async function isUserGame(req, res, next) {
	...
	if (!key)
		return res
			.status(401)
			.json({ res: res.statusCode, message: 'Unauthorized. Please provide a key.' })
	if (!steamIds[key])
		return res
			.status(401)
			.json({
				res: res.statusCode,
				message: `Unauthorized. Get yourself a key on ${req.app.locals.config.domain}/key`,
			})
...
async function log(logs_message, discord_message) {
	fs.writeFile(
		'data/logs.log',
		`[${new Date(Date.now()).toLocaleString('ru-RU')}] - ${logs_message}\n`,
		{ flag: 'a' },
		async err => {
			if (err) throw err
